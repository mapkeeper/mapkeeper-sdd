    ...
    <section className="voice-panel" aria-label="음성 입력">
      ...
      {state === 'IDLE' ? (
        <button className="voice-panel__primary" type="button" onClick={onStart} aria-label="음성 인식 시작" style={{ minHeight: 56 }}>
          <span className="voice-panel__waves" aria-hidden="true"><i /><i /><i /><b>●</b><i /><i /><i /></span>
          <span>마이크를 눌러<br />말씀해 주세요</span>
        </button>
      ) : null}

      {state === 'LISTENING' ? <div className="voice-panel__listening"><span className="voice-panel__pulse" aria-hidden="true">●</span><p className="voice-panel__hint">말을 마치면 자동으로 정리해 드려요.</p></div> : null}
